...
if [[ "$visual_occlusion" = false ]]; then
  mkdir -p temp/results/reward-CyclicalJellyBeansOnions:agent-fov-$agent_fov:no-visual-occlusion/PPO/Vision+Scent/Plain
else
  mkdir -p temp/results/reward-CyclicalJellyBeansOnions:agent-fov-$agent_fov/PPO/Vision+Scent/Plain
fi
