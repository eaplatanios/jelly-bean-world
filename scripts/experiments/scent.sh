...
if [[ "$visual_occlusion" = false ]]; then
  mkdir -p temp/results/reward-CollectOnions:agent-fov-$agent_fov:no-visual-occlusion/PPO/Scent/Plain
else
  mkdir -p temp/results/reward-CollectOnions:agent-fov-$agent_fov/PPO/Scent/Plain
fi
