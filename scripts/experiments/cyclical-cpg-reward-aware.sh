...
if [[ "$visual_occlusion" = false ]]; then
  mkdir -p temp/results-cpg/reward-CyclicalJellyBeansOnions:agent-fov-$agent_fov:no-visual-occlusion/PPO/Vision+Scent/RewardAware
else
  mkdir -p temp/results-cpg/reward-CyclicalJellyBeansOnions:agent-fov-$agent_fov/PPO/Vision+Scent/RewardAware
fi
